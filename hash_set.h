...
public:
    hash_set(hash_set && hs) {
        size_of_table = hs.size_of_table; // размерность массива хеш таблицы
        arr = hs.arr;  // хеш таблица 
        count_of_element = hs.count_of_element; // кол-во элементов в множестве
        __head = hs.__head; // указатель на начало списка 
        __end = hs.__end; // указатель на конец списка 
        hs.size_of_table = 0;
        hs.arr = nullptr;
        hs.count_of_element = 0;
        hs.__head = nullptr;
        hs.__end = nullptr;
    }
